'use client'
import { ArrowDown, ArrowUp, Plus, Trash2 } from 'lucide-react'
import type { MasterCV } from '@/lib/documents/types'
import { Button } from '@/components/ui/button'
import { Card, CardContent } from '@/components/ui/card'
import { Input } from '@/components/ui/input'
import { Label } from '@/components/ui/label'

type Education = NonNullable<MasterCV['education']>[number]

interface EducationTabProps {
  cv: MasterCV
  setCv: (cv: MasterCV) => void
}

function empty(): Education {
  return { school: '', degree: '', start: '', end: '', location: '', honors: '' }
}

export function EducationTab({ cv, setCv }: EducationTabProps) {
  const items = cv.education ?? []

  function replace(next: Education[]): void {
    setCv({ ...cv, education: next })
  }

  function update(index: number, patch: Partial<Education>): void {
    replace(items.map((it, i) => (i === index ? { ...it, ...patch } : it)))
  }

  function move(index: number, direction: -1 | 1): void {
    const target = index + direction
    if (target < 0 || target >= items.length) return
    const next = [...items]
    const [it] = next.splice(index, 1)
    if (it) next.splice(target, 0, it)
    replace(next)
  }

  function remove(index: number): void {
    replace(items.filter((_, i) => i !== index))
  }

  function add(): void {
    replace([...items, empty()])
  }

  return (
    <div className="space-y-3">
      {items.length === 0 ? (
        <Card>
          <CardContent className="py-8 text-center text-sm text-muted-foreground">
            No education entries yet.
          </CardContent>
        </Card>
      ) : (
        items.map((it, i) => (
          <Card key={i}>
            <CardContent className="space-y-3 pt-6">
              <div className="flex items-start justify-between gap-2">
                <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                  Entry #{i + 1}
                </div>
                <div className="flex items-center gap-1">
                  <Button
                    type="button"
                    variant="ghost"
                    size="icon"
                    disabled={i === 0}
                    onClick={() => move(i, -1)}
                    aria-label="Move up"
                  >
                    <ArrowUp className="size-4" />
                  </Button>
                  <Button
                    type="button"
                    variant="ghost"
                    size="icon"
                    disabled={i === items.length - 1}
                    onClick={() => move(i, 1)}
                    aria-label="Move down"
                  >
                    <ArrowDown className="size-4" />
                  </Button>
                  <Button
                    type="button"
                    variant="ghost"
                    size="icon"
                    onClick={() => remove(i)}
                    aria-label="Delete"
                  >
                    <Trash2 className="size-4" />
                  </Button>
                </div>
              </div>
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="space-y-1.5">
                  <Label htmlFor={`edu-${i}-school`}>School</Label>
                  <Input
                    id={`edu-${i}-school`}
                    value={it.school}
                    onChange={(e) => update(i, { school: e.target.value })}
                  />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor={`edu-${i}-degree`}>Degree</Label>
                  <Input
                    id={`edu-${i}-degree`}
                    value={it.degree}
                    onChange={(e) => update(i, { degree: e.target.value })}
                  />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor={`edu-${i}-start`}>Start</Label>
                  <Input
                    id={`edu-${i}-start`}
                    value={it.start ?? ''}
                    placeholder="2016"
                    onChange={(e) => update(i, { start: e.target.value })}
                  />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor={`edu-${i}-end`}>End</Label>
                  <Input
                    id={`edu-${i}-end`}
                    value={it.end ?? ''}
                    placeholder="2020"
                    onChange={(e) => update(i, { end: e.target.value })}
                  />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor={`edu-${i}-location`}>Location</Label>
                  <Input
                    id={`edu-${i}-location`}
                    value={it.location ?? ''}
                    onChange={(e) => update(i, { location: e.target.value })}
                  />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor={`edu-${i}-honors`}>Honors</Label>
                  <Input
                    id={`edu-${i}-honors`}
                    value={it.honors ?? ''}
                    onChange={(e) => update(i, { honors: e.target.value })}
                  />
                </div>
              </div>
            </CardContent>
          </Card>
        ))
      )}
      <div className="flex justify-end">
        <Button type="button" variant="outline" onClick={add} size="sm">
          <Plus className="size-4" />
          Add education
        </Button>
      </div>
    </div>
  )
}
